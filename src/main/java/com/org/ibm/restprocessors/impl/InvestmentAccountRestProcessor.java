...
@Service
public class InvestmentAccountRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(InvestmentAccountRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.investmentaccount.hosturl}")
	private String url;
	
	@Value("${spring.api.investmentaccount.context-path}")
	private String contextPath;

	@Value("${spring.api.investmentaccount.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  InvestmentAccountRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ InvestmentAccountRestProcessor.class.getName());
