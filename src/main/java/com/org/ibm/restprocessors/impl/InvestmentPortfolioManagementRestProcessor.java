...
@Service
public class InvestmentPortfolioManagementRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(InvestmentPortfolioManagementRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.investmentportfoliomanagement.hosturl}")
	private String url;
	
	@Value("${spring.api.investmentportfoliomanagement.context-path}")
	private String contextPath;

	@Value("${spring.api.investmentportfoliomanagement.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  InvestmentPortfolioManagementRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ InvestmentPortfolioManagementRestProcessor.class.getName());
